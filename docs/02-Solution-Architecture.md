...
Three subscriptions or three resource groups with strict separation: `dev`, `uat`, `prd`. Same Terraform, different `.tfvars` — but dev and uat/prd deliberately run different **network topologies**, controlled by a `use_private_endpoints` variable on `modules/network`, `modules/sql`, `modules/keyvault`, `modules/storage` and `modules/functions`.

**Dev trades private endpoints for IP-restricted public access.** `use_private_endpoints = false`: no private endpoints, no private DNS zones, no VNet links on SQL, Key Vault or either storage account. Instead each falls back to `public_network_access_enabled = true` locked down to specific deployer IPs — `network_acls`/`network_rules` `default_action = "Deny"` with a non-empty `ip_rules` (Key Vault, Storage), or an `azurerm_mssql_firewall_rule` per address (SQL, which has no equivalent inline block). App Service and Functions still VNet-integrate into the app subnet, so the app itself runs in-subnet regardless. This exists so dev can be deployed from a developer workstation: a private endpoint is invisible to anything not already on the VNet, and dev has no VNet-resident deploy agent. `policy/terraform/azure_security.rego` enforces the shape of the exception (deny wherever `default_action` isn't `Deny` or `ip_rules` is empty) and denies it outright the moment `environment != "dev"`.

**UAT is a faithful, full-scale-topology mirror of production**, not merely "smaller." `infra/terraform/environments/uat/` sets `use_private_endpoints = true` everywhere: full private endpoints, private DNS zones and VNet links on every data service, no public access, no deployer IP exception. SKUs are scaled down from prod, but the network shape — the part that actually breaks in production the first time nobody's tested it — is identical. This is deliberate: a private-endpoint problem that only appears in production is the classic way this kind of project slips a month, and uat is where it gets caught instead. Prod uses the same topology as uat.

**UAT and production deployment both require a self-hosted runner inside the app subnet.** SQL, Key Vault and both storage accounts have no public endpoint there (§5) — they are reachable only over their private endpoints from that subnet. A GitHub-hosted runner cannot reach a private endpoint at all, and its IP rotates per job, so it cannot be admitted through an `ip_rules` allow-list either. `policy/terraform/azure_security.rego` denies public network access on SQL, Key Vault, Storage and (once one exists) Service Bus unconditionally outside dev, and separately requires a private endpoint be present for each. Provision the runner as an Azure VM or VM Scale Set inside the app subnet, registered against this repository, before the first uat or production apply.

**Dev CI deploys are manual, or run from the same self-hosted runner.** A GitHub-hosted runner's egress IPs aren't fixed to a small, stable range that's practical to allowlist in `deployer_ip_addresses` — they're drawn from Azure/GitHub's published (and periodically rotated) datacenter ranges, so admitting them narrows the exception in name only. Dev therefore isn't deployed from GitHub-hosted CI; apply it manually from a developer workstation (adding that workstation's egress IP to `deployer_ip_addresses`) or from the same self-hosted runner used for uat/prd.
